...
<html>
  <head></head>
  <body>
    <a href="llama-cpp-python-cpu/">llama-cpp-python-cpu</a>
    <br>
  </body>
</html>
...
mkdir -p "$output_dir/llama-cpp-python-cpu"

# Create an index html file in llama-cpp-python directory
cat << EOF > "$output_dir/llama-cpp-python-cpu/index.html"
<!DOCTYPE html>
<html>
  <body>
...
for release in $releases; do
    ...
    response=$(curl -s "${headers[@]}" \
                    "https://api.github.com/repos/TheBigEye/llama-cpp-python-cpu/releases/tags/$release")
    ...
    echo "    <h2>$release_version</h2>" >> "$output_dir/llama-cpp-python-cpu/index.html"
    ...
    echo "$wheel_urls" | while read -r asset; do
        echo "    <a href=\"$asset\">$asset</a>" >> "$output_dir/llama-cpp-python-cpu/index.html"
        echo "    <br>" >> "$output_dir/llama-cpp-python-cpu/index.html"
    done
done

echo "  </body>" >> "$output_dir/llama-cpp-python-cpu/index.html"
echo "</html>" >> "$output_dir/llama-cpp-python-cpu/index.html"
echo "" >> "$output_dir/llama-cpp-python-cpu/index.html"
